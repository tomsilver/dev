...
class RepositioningPlanner(abc.ABC):
    ...
    def run(self, initial_state: RepositioningState,
            goal_state: RepositioningState,
            warm_start: bool = True) -> list[JointTorques]:
        """Return an open-loop plan. Note that the planner can be used with
        MPC by just calling every timestep with warm_start=True."""
        self._last_plan = self._run(initial_state, goal_state, warm_start=warm_start)
        return list(self._last_plan)

    @abc.abstractmethod
    def _run(self, initial_state: RepositioningState,
            goal_state: RepositioningState,
            warm_start: bool = True) -> list[JointTorques]:
