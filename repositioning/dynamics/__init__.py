...
from .pybullet_constraint_model import PybulletConstraintRepositioningDynamicsModel
...
def create_robot(name: str, physics_client_id: int, base_pose: Pose, joint_positions: JointPositions) -> SingleArmPyBulletRobot:
    """Create a robot from its name."""
    if name == "human-arm-6dof":
        return HumanArm6DoF(physics_client_id, base_pose=base_pose, home_joint_positions=joint_positions)

    if name == "panda-limb-repo":
        return PandaPybulletRobotLimbRepo(physics_client_id, base_pose=base_pose, home_joint_positions=joint_positions)
    
    return create_pybullet_robot(name, physics_client_id, base_pose=base_pose,
...
def create_dynamics_model(
    ...
    active_arm = create_robot(scene_config.active_name, physics_client_id, scene_config.active_base_pose)
