...
def enumerate_from_grammar(grammar, start_symbol=0, num_iterations=100000000):
    queue = []
    ...
    for iteration in range(num_iterations):
        ...
        for child_program, child_production_prob, child_priority in get_child_programs(program, grammar):
            if program_is_complete(child_program):
                yield Feature(stringify(child_program)), -production_neg_log_prob + np.log(child_production_prob)
            else:
